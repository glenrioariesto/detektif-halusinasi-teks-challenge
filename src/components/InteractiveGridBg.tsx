    ...
    const draw = () => {
      ...
      for (const d of drops) {
        ...
        for (let j = 0; j < d.chars.length; j++) {
          ...
          let drawX = d.x;
          let drawY = cy;
          // Rose Pink as matrix rain default color (user preferred)
          let color = `rgba(244, 63, 94, ${d.opacity * (1 - j / d.chars.length)})`;
          let scale = 1;

          // Glitch scramble timer triggered on click (Neon Rose / Coral Pink)
          if (glitchTimerRef.current > 0) {
            color = `rgba(251, 113, 133, ${Math.random() * 0.8})`;
          ...
          if (dist < warpRadius) {
            ...
            // Brighten up and change color to light rose pink when near cursor
            color = `rgba(251, 113, 133, ${0.15 + force * 0.85})`;
            scale = 1.0 + force * 0.2;
      ...
      // Draw cursor scanning reticle overlay (Neon Rose)
